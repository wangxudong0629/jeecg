...
<div id="contbox" style="float:left;color:white;margin:22px 75px 0 75px; overflow:hidden;font: 17px Arial,Helvetica;color:white">
    <p id="updatePlan" style="color: skyblue;display: none"></p>
</div>
<div style="color: red;width: 300px;float:right" >
    <div>
        <span id="updateCount" style="font-size: 14px" >0</span><span  id="updateMessage" style="font-size: 12px">条操作未保存！</span>
        <input type="button" value="保存" onclick="updateGantt()">
        <input type="button" value="重置" onclick="resetGanttData()">
    </div>
</div>
<div id="gantt_here" style="width:100%; height:100%;"></div>
<script type="text/javascript">
    function  resetGanttData() {
        //$("#ganttFrame").href("planController.do?gantt");
        window.location="planController.do?gantt";
    }
    ...
    function loadGantt() {
        jQuery.ajax({
            ...
            type: 'POST',
            dataType : "json",
            data:{id:'${plan.getId()}',number:'${plan.getNumber()}'},
    ...
    function  showGantt(demo_tasks) {
        ...
        gantt.attachEvent("onAfterTaskDrag", function (id, mode) {
            ...
            if (mode == gantt.config.drag_mode.progress) {
                ...
                var t=task.end_date.getTime()-1000*60*60*24;
                var lt=new Date(t);
                var e=gantt.templates.date_grid(lt);
                //gantt.message(task.id+task.text + " 开始时间：" + s + " 结束时间： " + e);
                $("#updatePlan").append(task.id+"," + s + "," + e+",");
                var count =$("#updateCount").text();
                //$("#updateMessage").removeAttr("style");
                $("#updateCount").text(parseInt(count)+parseInt(1));
            }
        });
