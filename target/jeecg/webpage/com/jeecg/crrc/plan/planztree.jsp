...
<div class="easyui-layout" fit="true" scroll="no">
    <div  data-options="region:'west',title:'项目管理',split:true" style="width:200px;overflow: auto;">
<div>
        <div style="width:105px;margin-left: 8px;margin-top: 2px;float: left">
            <a  icon="icon-add" class="easyui-linkbutton l-btn l-btn-plain"  onclick="addOneNode()">
                <span class="bigger-110 no-text-shadow" style="width: 50px;">添加项目</span>
            </a>
        </div>
    ...
    <div class="hidden">
        <div id="orgMenu" class="easyui-menu" data-options="onClick:menuHandler" style="width: 120px;">
            <div data-options="name:'add'">添加子级</div>
...
<script>
    ...
    function loadTree() {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            dataType : "json",
            url: 'planController.do?getTreeDemoData',//请求的action路径
        ...
        var ztreeCreator = new ZtreeCreator('orgTree',"planController.do?getTreeData",zNodes)
    ...
    function zTreeOnLeftClick(event, treeId, treeNode) {
        ...
        alert("curSelectNode"+curSelectNode);
        alert("treeNode.id"+treeNode.id);
        alert("parentId"+curSelectNode.parentId);
        alert("curSelectNode.id"+curSelectNode.id);
        var parentId = treeNode.id;
        var url1 = "planController.do?we&id=" + curSelectNode.id;
        var url2 = "planController.do?ig&id=" + curSelectNode.id;
    ...
    //添加一级节点
    function addOneNode() {
        var url = "planController.do?toAddproject";
    ...
    function addNode() {
        ...
        var url = "planController.do?goAdd&id=" + selectNode.id;
        $("#listFrame").attr("src", url);
    ...
    function editNode(type) {
        ...
        //根节点 不能编辑
        if(selectNode.id=="0"  ){
            var url = "planController.do?goUpdate1&id=" + selectNode.id;
            $("#listFrame").attr("src", url);
        }else {
        var url = "planController.do?goUpdate&id=" + selectNode.id;
        $("#listFrame").attr("src", url);
        }
    };
    ...
    function delNode() {
        ...
        var url = "planController.do?checkifhaveparent&id=" + nodeId;
        jQuery.ajax({
            async: false,
            cache: false,
            type: 'GET',
            ...
            success: function (data) { //请求成功后处理函数。
                if (data.success) {
                    ...
                    // }else {
                    if(data.msg=="1"){
                        var url = "planController.do?del&id=" + nodeId;
                        ...
                        var url = "planController.do?del2&id=" + nodeId;
